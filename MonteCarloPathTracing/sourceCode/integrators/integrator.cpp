...
Spectrum Integrator::Li(const Scene * scene, const Ray & ray)
{
	...
	if (isect.isValid) {
		...
		// diffuse
		//if (!is_hit_face_back && diffuse>FLOAT_EPSILON){// && u0<diffuse) {
		if (diffuse>FLOAT_EPSILON && material.Tr < FLOAT_EPSILON) {// && u0<diffuse) {
		...
		//specular
		//if (!is_hit_face_back && specular > FLOAT_EPSILON){// && u0 < diffuse + specular) {
		if (specular > FLOAT_EPSILON) {// && u0 < diffuse + specular) {
			if (is_hit_face_back) isect.normal = -isect.normal;
		...
		if (material.Tr > FLOAT_EPSILON) {
			...
			//spec += Li(scene, next_ray)*material.Tr;
		}
		
		spec = spec* p;

		// ambient mainly Emitter
		//if (!is_hit_face_back)
			spec += material.Ka;// +material.Kd*0.5f;
